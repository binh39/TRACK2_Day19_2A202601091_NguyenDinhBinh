...
feast_cli = str(Path(sys.executable).with_name("feast.exe"))
subprocess.run([sys.executable, str(ROOT / "scripts" / "gen_spend.py")], check=True,
               capture_output=True)
subprocess.run([feast_cli, "apply"], cwd=repo, check=True, capture_output=True)
subprocess.run([feast_cli, "materialize-incremental", "2027-01-01T00:00:00"],
